...
for vRandSeed in vRandSeedList:
    ...
    # result_gene_auto = np.concatenate((y_valid_gProb , y_valid_AProb),axis=1)
    # result_gene_autoTEST = np.concatenate((y_test_gProb , y_test_AProb),axis=1)
    # print(result_gene_auto.shape)
    # print(result_gene_autoTEST.shape)
    ...
    # # model = Sequential()
    # # model.add(Dense(num_classes*2, input_dim=result_gene_auto.shape[1], activation='relu'))  # Single hidden layer with 64 neurons
    ...
    # # # Train the model
    # # model.fit(result_gene_auto, y_valid_categorical, epochs=vFinalEpo, batch_size=32)
    ...
    # model = XGBClassifier(max_depth=4, learning_rate=0.3, n_estimators=vFinalEpo, 
    #                   objective='multi:softmax',num_class=num_classes
    #                   , booster='gbtree',verbosity=1,
    #                   eval_metric='merror',tree_method="hist")
    # model.fit(result_gene_auto, y_valid)
    
    # from sklearn.metrics import accuracy_score
    # #y_test_categorical = to_categorical(y_test, num_classes=num_classes)
    # predicted_classes = model.predict(result_gene_autoTEST)
